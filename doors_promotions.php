...
function force_check_for_promo_plugin_updates()
{
	if (isset($_GET['update_promotions']) && $_GET['update_promotions'] === '1') {
		...
		$update_url = admin_url("update.php?action=upgrade-plugin&plugin=doors_promotions%2Fdoors_promotions.php&_wpnonce=$nonce");
		wp_redirect($update_url);
		exit;
	}
}
add_action('admin_init', 'force_check_for_promo_plugin_updates');
